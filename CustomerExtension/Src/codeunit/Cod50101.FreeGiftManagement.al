codeunit 50101 "FreeGiftManagement"
{
    ...
    begin
        //if (Rec.Type = Rec.Type::Item) and (IsCustomerGOLD(Rec."Sell-to Customer No.")) then begin
            ...
            if FreeGift.Get(Customer."Customer Category", Rec."No.") and (rec.Quantity > FreeGift.MinimumOrderQuantity) then begin
    ...
    var
        Customer: Record Customer;
    begin
        if Customer.Get(CustomerNo) and (Customer."Customer Category" = 'GOLD') then
            exit(true)
        else
            exit(false);
    end;
